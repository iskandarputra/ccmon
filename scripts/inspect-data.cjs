...
#!/usr/bin/env node
/* Dev utility: inspect the shape of Claude Code's local data files. */
'use strict';
...
for (const dir of fs.readdirSync(root)) {
  ...
  let st;
  try { st = fs.statSync(dirPath); } catch { continue; }
...
for (const l of lines) {
  let j;
  try { j = JSON.parse(l); } catch { types.MALFORMED = (types.MALFORMED || 0) + 1; continue; }
