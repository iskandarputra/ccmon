...
describe('resolveRange — calendar presets', () => {
  it('month is first-of-month through today', () => {
    expect(resolveRange({ preset: 'month' }, NOW)).toEqual({
      preset: 'month', startKey: '2026-06-01', endKey: '2026-06-19', label: 'Jun 2026',
    });
  });
  it('lastMonth snaps to the full previous calendar month', () => {
    expect(resolveRange({ preset: 'lastMonth' }, NOW)).toEqual({
      preset: 'lastMonth', startKey: '2026-05-01', endKey: '2026-05-31', label: 'May 2026',
    });
  });
  it('lastMonth handles the year boundary', () => {
    ...
    expect(resolveRange({ preset: 'lastMonth' }, jan)).toMatchObject({
      startKey: '2025-12-01', endKey: '2025-12-31', label: 'Dec 2025',
    });
  });
});
...
describe('resolveRange — all + custom', () => {
  it('all is unbounded', () => {
    expect(resolveRange({ preset: 'all' }, NOW)).toEqual({
      preset: 'all', startKey: null, endKey: null, label: 'all time',
    });
  });
  it('custom keeps explicit bounds and labels them', () => {
    expect(resolveRange({ preset: 'custom', customStart: '2026-01-05', customEnd: '2026-02-10' }, NOW)).toEqual({
      preset: 'custom', startKey: '2026-01-05', endKey: '2026-02-10', label: 'Jan 5 – Feb 10',
    });
  });
  it('custom normalizes reversed bounds', () => {
    expect(resolveRange({ preset: 'custom', customStart: '2026-02-10', customEnd: '2026-01-05' }, NOW)).toMatchObject({
      startKey: '2026-01-05', endKey: '2026-02-10',
    });
  });
  it('custom with one open end labels directionally', () => {
    expect(resolveRange({ preset: 'custom', customStart: '2026-03-01', customEnd: null }, NOW).label).toBe('since Mar 1');
  });
});

describe('dayKeyInRange + isBoundedRange', () => {
  const r: ResolvedRange = { preset: '30d', startKey: '2026-05-21', endKey: '2026-06-19', label: 'last 30 days' };
