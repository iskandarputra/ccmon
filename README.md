...
About the icon: it ships as code, not pixels. `npm run icon` draws a 1024px
PNG from scratch in pure Node, and electron-builder derives the Windows
`.ico` and macOS `.icns` from it at package time. The `.deb` and AppImage
use the PNG directly. Every build script runs this step for you.
